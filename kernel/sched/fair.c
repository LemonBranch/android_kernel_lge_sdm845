...
unsigned int sysctl_sched_capacity_margin_down = 1280; /* ~20% margin */
				   ...
				   bool sync_boost)
{
	...
	next_cpu = find_best_target(p, &backup_cpu, boosted, sync_boost,
				    prefer_idle, &fbt_env, prefer_high_cap);
	if (next_cpu == -1) {
		target_cpu = prev_cpu;
		goto out;
	}
	...
	if ((prefer_high_cap || prefer_idle) && idle_cpu(next_cpu)) {
...
select_task_rq_fair(struct task_struct *p, int prev_cpu, int sd_flag, int wake_flags)
{
	...
	if (energy_aware()) {
		...
		*/
		...
		rcu_read_lock();
		new_cpu = select_energy_cpu_brute(p, prev_cpu, sync, sync_boost);
		rcu_read_unlock();
		return new_cpu;
	}
...
void check_for_migration(struct rq *rq, struct task_struct *p)
{
	...
	if (rq->misfit_task) {
		...
		rcu_read_lock();
		new_cpu = select_energy_cpu_brute(p, cpu, 0, false);
		rcu_read_unlock();
